...
export default function OrganizationAnalyticsPage() {
  ...
  const loadAnalytics = async () => {
    try {
      ...
      const [analyticsResponse, organizationResponse] = await Promise.all([
        apiRequest(`/analytics/organizations/${organizationId}`),
    ...
    <main className="min-h-screen bg-slate-950 text-white">
      <header className="border-b border-slate-800 bg-slate-950/80">
        <div className="max-w-6xl mx-auto px-4 py-5 flex items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold mt-2">Organization Analytics</h1>

            <p className="text-slate-400 mt-1">
              {organization?.name || "Workspace"} performance overview.
            </p>
          </div>
      ...
      <section className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <MetricCard label="Projects" value={analytics?.overview?.totalProjects || 0} />
          <MetricCard label="Members" value={analytics?.overview?.totalMembers || 0} />
          <MetricCard label="Total tasks" value={analytics?.overview?.totalTasks || 0} />
          <MetricCard label="Overdue tasks" value={analytics?.overview?.overdueTasks || 0} />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4">
          <MetricCard label="Completed tasks" value={analytics?.overview?.completedTasks || 0} />
          <MetricCard label="Active tasks" value={analytics?.overview?.activeTasks || 0} />
        ...
        <section className="mt-8 rounded-2xl bg-slate-900 border border-slate-800 p-6">
          ...
          <p className="text-slate-400 mt-2">
            Weekly completion trend for the last few weeks.
          </p>
        ...
        <section className="mt-8 rounded-2xl bg-slate-900 border border-slate-800 p-6">
          ...
          <p className="text-slate-400 mt-2">
            See who has active, completed, and overdue tasks.
          </p>
